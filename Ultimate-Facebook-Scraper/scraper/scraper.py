...
#Aditional libraries required to go incognito mode
...
# Auxiliar functions to go incognito
...
def get_proxies(proxies):
    ...
    print('There are ' + str(len(proxies)) + ' proxies available.')
    print('Updating proxies')

    proxies_req = Request('https://www.sslproxies.org/') # request
    proxies_req.add_header('User-Agent', ua.random) # use fake user agent
    proxies_doc = urlopen(proxies_req).read().decode('utf8') # open

    soup = BeautifulSoup(proxies_doc, 'html.parser')
    proxies_table = soup.find(id='proxylisttable') #get proxies table
...
def login(email, password):
    ...
    try:
        ...
        proxies = [] # contain proxies: [ip, port]
        ...
        # Changing proxy argument in webdriver
